...
export function ResumeBulletAnalyzer(): JSX.Element {
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
      <label htmlFor="rba" className="text-sm font-bold text-slate-800">Paste your resume bullet points — one per line</label>
      <textarea
        id="rba"
        value={text}
        ...
        className="mt-2 h-44 w-full resize-y rounded-xl border border-slate-300 bg-slate-50 p-3 text-sm leading-6 text-slate-800 outline-none focus:border-brand focus:ring-2 focus:ring-brand/20"
      />
      ...
      {results ? (
        <div id="rba-result" className="mt-8 border-t border-slate-100 pt-8">
          <p className="text-sm font-semibold text-slate-500">{results.length} bullet{results.length === 1 ? '' : 's'} analysed · average strength <span className="font-bold text-slate-900">{avg}/100</span></p>
          <div className="mt-5 space-y-4">
            {results.map((r, i) => (
              <div key={`${i}-${r.text.slice(0, 20)}`} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <p className="text-sm leading-6 text-slate-800">{r.text}</p>
