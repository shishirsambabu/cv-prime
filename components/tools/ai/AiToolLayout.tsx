...
const SITE_URL = 'https://cv-prime.in';
...
export function AiToolLayout({
  ...
  highlight: string;
  subtitle: string;
  /** Structured-data identity for this tool page — path is the route below /tools, e.g. "ai-ats-score". */
  schema?: { name: string; description: string; path: string };
  children: ReactNode;
}): JSX.Element {
  const url = schema ? `${SITE_URL}/tools/${schema.path}` : undefined;
