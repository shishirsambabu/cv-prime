 ...
 *
 * `path`, `appName`, `appDescription`, and `faqs` are optional so the
 * component still renders for any caller that predates them, but every
 * current tool page passes them to get FAQ + WebApplication + breadcrumb
 * JSON-LD and an on-page FAQ section for AI/search citation.
 */
export function AiToolLayout({
  ...
  const url = path ? `https://cv-prime.in${path}` : undefined;
  const faqSchema =
    faqs && faqs.length > 0
      ? {
  ...
  const appSchema =
    url && appName
      ? {
          ...
          operatingSystem: 'Web',
          description: appDescription ?? subtitle,
