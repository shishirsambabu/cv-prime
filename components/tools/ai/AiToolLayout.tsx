 ...
 *
 * `slug`, `appName`, and `faqs` are optional so existing callers keep working,
 * but every tool page should pass them: they drive the FAQ section plus the
 * FAQPage/SoftwareApplication/BreadcrumbList JSON-LD used for AI/search citation.
 */
export function AiToolLayout({
  ...
  const url = slug ? `https://cv-prime.in/tools/${slug}` : undefined;
  const plainTitle = `${title} ${highlight}`.replace(/\s+/g, ' ').trim();
  ...
  const breadcrumbSchema = url
    ? {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: appName ?? plainTitle, item: url },
        ],
      }
    : null;
      ...
      {faqs && faqs.length > 0 && (
        <section className="border-t border-white/10 px-5 py-16">
          <div className="mx-auto max-w-3xl">
            <h2 className="text-center font-display text-2xl font-bold sm:text-3xl">{plainTitle} — FAQ</h2>
