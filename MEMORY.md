...
- RESOLVED: `lib/roleData.ts` has 51 roles (added `ai-engineer` in a later pass, bringing the count up from 50). `lib/salaryData.ts`, `lib/coverLetterData.ts`, and `lib/atsGuideData.ts` previously only hand-curated 36 of them; the 15 gap roles (business-development-manager, chartered-accountant, embedded-systems-engineer, full-stack-developer, interior-designer, investment-banker, ios-developer, logistics-manager, machine-learning-engineer, network-engineer, pharmacist, react-developer, sales-executive, sap-consultant, scrum-master) now have hand-written entries in all three files, anchored by analogy to an existing comparable role's real numbers/tone (e.g. `full-stack-developer` anchored on `software-engineer`, `chartered-accountant` on `finance-analyst`/`accountant`) rather than fabricated from nothing. `sitemap.ts`'s `salaryRoleSlugs`/`atsGuideRoleSlugs`/`coverLetterRoleSlugs` filters picked them up automatically — `/salary/[role]`, `/ats-guide/[role]`, and `/cover-letter-examples/[role]` each now statically generate all 51 roles (verified via `npm run build`), closing 45 previously-404ing pages. Also fixed the site-wide "50 roles" copy (which had drifted stale after the `ai-engineer` addition) to "51 roles" across `public/llm.txt`/`llms.txt`, several `cv-prime-vs-*` comparison pages, `in-demand-skills-india-2026`, `resume-maker`, `resume-generator`, `cover-letter-generator`, `best-resume-builders-india`, and `lib/blogData.ts`.
