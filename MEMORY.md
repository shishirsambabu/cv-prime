...
- Growth-engineering audit: closed the 15-role salary/cover-letter/ATS-guide gap noted below by adding `generateStubSalaryData`/`generateStubCoverLetterData`/`generateStubAtsGuideData` to `lib/salaryData.ts`/`lib/coverLetterData.ts`/`lib/atsGuideData.ts`. Each stub is derived only from the role's own already-published `roleData.ts` fields (`salaryRange`, `keySkills`, `topCompanies`, `industry`, `whatToInclude`) and uses qualitative/comparative language instead of inventing new precise salary figures — following the guidance the previous pass left below. A loop at the bottom of each file fills in any `roleSlugs` entry missing from the hand-curated map, so `sitemap.ts`'s existing `salaryRoleSlugs`/`atsGuideRoleSlugs`/`coverLetterRoleSlugs` filters now include all 51 roles automatically (36 hand-curated + 15 stub-generated), and any future role added to `roleData.ts` gets a stub page for free without further agent work. Added `lowerRoleName`, `articleFor`, and `pluralRoleName` helpers to `lib/roleData.ts` so mid-sentence interpolation doesn't mangle acronym titles (`iOS Developer`, `SAP Consultant`, `Chartered Accountant (CA)`) or pick the wrong "a"/"an".
