...
- SEO/GEO growth-engineer audit fixed a site-wide title-template bug (root layout appended "| CV Prime" on top of page titles that already had it, doubling the suffix on ~800+ rendered `<title>` tags including the homepage); root layout title is now a plain string with no template. Also fixed: robots.ts now disallows `/auth/`, `/share/`, `/ai-cv/`, `/cover-letter/` (previously crawlable app/auth surfaces); sitemap.ts adds the missing `/cv-prime-vs-linkedin-resume` entry and drops the pure-redirect `/create-with-ai` entry; added WebApplication + BreadcrumbList JSON-LD to all 12 `/tools/ai-*` pages via a shared `AiToolLayout` schema prop; standardized the mislabeled "Compare" breadcrumb node (was pointing at `/templates` on 13 pages) to the real `/resume-tools` hub across all 18 comparison pages, and added full Article/FAQ/Breadcrumb schema to the 2 comparison pages that had none; footer now links all 8 city pages and all 18 comparison pages (previously only 7 were linked) plus both glossary pages; `public/llm.txt` now lists the 7 pages missing from the last content batch; fixed a stray "Secured by Cashfree" vendor name on the homepage pricing CTA that a pre-existing test (`publicProviderReferences.test.ts`) flags as public copy that shouldn't name the payment processor.
