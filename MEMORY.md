...
- **CRITICAL PROCESS GAP (found 2026-07-11): dozens of daily Growth-OS session branches (`claude/great-feynman-*`, `claude/magical-thompson-*`, etc.) have been pushed to origin but never merged into `main` since at least 2026-06-13.** Several of these branches independently rediscovered and "fixed" the same bugs (missing `/cv-prime-vs-linkedin-resume` sitemap entry, stale "35 roles" copy, fabricated schema rating) on isolated branches that were never merged, so `main`/production never received the fixes — the same bugs kept being rediscovered day after day. This session fixed the underlying bugs directly on `main`'s current state (see below) but did **not** merge or reconcile the other stranded branches — that needs a human decision (many are 100+ commits behind current main and diverged from an early pre-launch snapshot, so most are not worth recovering, but a few recent ones, e.g. `claude/great-feynman-l96sa1` and `claude/magical-thompson-hxwur3`, contain small legitimate fixes that were re-derived here anyway). Recommend either wiring the daily routine to open/auto-merge a PR against `main`, or deleting the stale branches after confirming nothing else of value is stranded on them.
- Growth-OS audit (2026-07-11): fixed `/cv-prime-vs-linkedin-resume` missing from `app/sitemap.ts` (page + robots.ts allow-list already existed, so it was crawlable but not being actively submitted for indexing). Removed a fabricated `aggregateRating` (4.8, 312 ratings) from the site-wide `SoftwareApplication` JSON-LD in `app/layout.tsx` — there is no review/testimonial data source backing this number anywhere in the codebase, and self-serving fake ratings violate Google's structured-data guidelines and risk a manual action against rich-result eligibility site-wide. Corrected stale "35 roles" copy to the accurate 49-role count across `app/layout.tsx` (featureList, plus removed a literal duplicate line) and 9 marketing pages (salary, interview-questions, linkedin-headline, resume-examples hub + role page, internship-resume, and 3 competitor comparison pages) — left the two legitimate "35 roles across 8 metros" mentions in `llm.txt` untouched since the role×city matrix intentionally covers a 35-role subset, not the full 49. Removed a dead `/resume-tips/references` link from `llm.txt` (page doesn't exist; the real page is `/resume-tips/references-on-resume`, already listed separately). `npm run build` and `npx tsc --noEmit` pass with no new errors.
