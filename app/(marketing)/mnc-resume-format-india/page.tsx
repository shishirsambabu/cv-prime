    ...
    <main className="min-h-screen bg-white text-slate-950">
      ...
      <section className="border-b border-slate-100 bg-slate-50 px-5 py-12">
        <div className="mx-auto max-w-5xl">
          <p className="text-center text-sm font-semibold uppercase tracking-wider text-slate-500">Built for the format these MNCs expect</p>
          <div className="mt-6 flex flex-wrap justify-center gap-3">
            {companies.map((c) => (
              <span key={c} className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700">{c}</span>
            ))}
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-5xl">
          ...
          <div className="mt-12 grid gap-5 sm:grid-cols-2">
            {rules.map((r) => {
              const Icon = r.icon;
              return (
                <div key={r.title} className="rounded-2xl border border-slate-200 bg-slate-50 p-6">
                  <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-brand/10 text-brand">
                    <Icon className="h-5 w-5" />
                  </div>
                  <h3 className="mt-4 font-display text-lg font-bold text-slate-950">{r.title}</h3>
